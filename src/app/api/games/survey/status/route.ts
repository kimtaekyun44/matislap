...
// GET /api/games/survey/status?room_id=xxx&participant_id=xxx - 설문 게임 현재 상태 조회
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('설문 상태 조회 API 오류:', error)
    return NextResponse.json(
      { error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    )
...
// POST /api/games/survey/status - 게임 시작 또는 종료 (강사 전용)
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      room: { id: updatedRoom.id, status: updatedRoom.status },
      total_questions: totalQuestions || 0
    })
  } catch (error) {
    console.error('설문 상태 변경 API 오류:', error)
    return NextResponse.json(
      { error: '서버 오류가 발생했습니다.' },
      { status: 500 }
    )
